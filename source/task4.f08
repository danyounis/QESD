program task4
    ...
    type(OptimizeND_NelderMead) :: Hnm, G
    ...
    allocate(My(nmins), Mp(nmins,adim))

    ! set $\rho$ 8-tuple
    r = RHO(time)

    call G%create(func=Gmax, ndim=vdim, ftol=ftol_e, itmax=itmax_e, warn=.false.)

    ! G%aux1: optimal angles for ea. Nelder-Mead vertex
    ! G%aux2: assists G%aux1 in assignment to the correct location
    allocate(G%aux1(vdim+1,adim), G%aux2(1,adim))
...
! see Task 4 notes for the density matrix description
function RHO(t)
    implicit none
    real(num) :: RHO(vdim)
    ...
    RHO(1) = q**2
    RHO(2) = (sin(Theta)**2)*(p**2)/dsqrt2
    RHO(3) = 0.
    RHO(4) = (cos(Theta)**2)*(p**2)
    RHO(5) = 0.
    RHO(6) = 0.
    RHO(7) = (sin(Theta)**2)*(p**2)/dsqrt2
    RHO(8) = dsqrt2*sin(Theta)*cos(Theta)*(p**2)
    RHO(9) = 0.
    RHO(10) = 0.
    RHO(11) = 0.
    RHO(12) = 0.
...
subroutine read_deck
    ...
    integer :: iu
    open(newunit=iu, file='input-T4.deck', status='old', action='read')
...
subroutine write_data
    ...
    do n=1,vdim+1
        grad(n,:) = r - TrPmatPSI(PSI(G%aux1(n,1:3),G%aux1(n,4:9),G%aux1(n,10:13),G%aux1(n,14)))
        write(iu,*) grad(n,:)
    end do
    close(iu)
