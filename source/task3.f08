...
! see Task 3 notes for the density matrix description
function RHO(t)
    implicit none
    real(num) :: RHO(vdim)
    ...
    RHO(1) = (s2 + c2*(q**6 + p**6))/dsqrt2
    RHO(2) = dsqrt2*s*c*(p**3)
    RHO(3) = 0.
    RHO(4) = (s2 + c2*(q**6 - p**6))/dsqrt2
    RHO(5) = dsqrt3*c2*(p**2)*(q**4)
    RHO(6) = dsqrt3*c2*(p**4)*(q**2)
...
function TrPmatPSI(S) result(p)
    ...
    p(5) = real((abs(S(2))**2 + abs(S(3))**2 + abs(S(5))**2)/dsqrt3, num)
    p(6) = real((abs(S(4))**2 + abs(S(6))**2 + abs(S(7))**2)/dsqrt3, num)
...
subroutine read_deck
    ...
    integer :: iu
    open(newunit=iu, file='input-T3.deck', status='old', action='read')
