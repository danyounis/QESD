program task7
    ...
    integer, parameter :: vdim = 22 ! witness operator dimensionality
    ...
    ! general variables
    real(num) :: time, Theta, tic, toc
    integer :: l, lm=1
    ! r: rho 22-tuple, v: witness operator 22-tuple
...
real(num) function Gmax(x)
    ...
    ! allows the user to inquire about the amoeba's current state (vertex values)
    inquire(file='inquire', exist=b_inquire)

    if (b_inquire) then
        open(newunit=iu, file='reply-feval.dat', status='replace')
        do n=1,vdim+1
            write(iu,*) -G(l)%y(n)
        end do
        close(iu)
        open(newunit=iu, file='reply-vstar.dat', status='replace')
        do n=1,vdim+1
            write(iu,*) G(l)%p(n,:)
        end do
        close(iu)
    ...
    ! allows the user to halt optimization and write all data
    inquire(file='halt', exist=b_halt)
...
! see Task 7 notes for the density matrix description
function RHO(t)
    implicit none
    real(num) :: RHO(vdim)
    ...
    RHO(1) = (q**4)*(sin(Theta)**2) + (q**6)*(cos(Theta)**2)
    RHO(2) = (2./dsqrt3)*(p*q*sin(Theta))**2 + dsqrt3*((q**2)*p*cos(Theta))**2
    RHO(3) = (p**4)*(sin(Theta)**2)/dsqrt3 + dsqrt3*((p**2)*q*cos(Theta))**2
    RHO(4) = (p**6)*(cos(Theta)**2)
    RHO(5) = (dsqrt2/dsqrt3)*(p*q*sin(Theta))**2
    RHO(6) = 0.
    RHO(7) = (dsqrt2/dsqrt3)*(p**4)*(sin(Theta)**2)
    RHO(8) = 0.
    RHO(9) = dsqrt2*p*(q**4)*sin(Theta)*cos(Theta)
    RHO(10) = 0.
    RHO(11) = 0.
    RHO(12) = 0.
    RHO(13) = 0.
    RHO(14) = 0.
    RHO(15) = 2.*(q**2)*(p**3)*sin(Theta)*cos(Theta)
    RHO(16) = 0.
    RHO(17) = 0.
    RHO(18) = 0.
    RHO(19) = 0.
    RHO(20) = 0.
    RHO(21) = dsqrt2*(p**5)*sin(Theta)*cos(Theta)
    RHO(22) = 0.
...
function TrPmatPSI(S) result(p)
    ...
    p(5) = real((1./dsqrt6)*(conjg(S(5))*(S(2)+S(3)) + conjg(S(3))*(S(2)+S(5)) + conjg(S(2))*(S(3)+S(5))), num)
    p(6) = real((i/dsqrt6)*(conjg(S(5))*(S(2)+S(3)) + conjg(S(3))*(S(2)-S(5)) - conjg(S(2))*(S(3)+S(5))), num)
    p(7) = real((1./dsqrt6)*(conjg(S(7))*(S(4)+S(6)) + conjg(S(6))*(S(4)+S(7)) + conjg(S(4))*(S(6)+S(7))), num)
    p(8) = real((i/dsqrt6)*(conjg(S(7))*(S(4)+S(6)) + conjg(S(6))*(S(4)-S(7)) - conjg(S(4))*(S(6)+S(7))), num)
    p(9) = real((1./dsqrt6)*((conjg(S(2))+conjg(S(3))+conjg(S(5)))*S(1) + conjg(S(1))*(S(2)+S(3)+S(5))), num)
    p(10) = real((i/dsqrt6)*((conjg(S(2))+conjg(S(3))+conjg(S(5)))*S(1) - conjg(S(1))*(S(2)+S(3)+S(5))), num)
    p(11) = real((1./dsqrt6)*((conjg(S(4))+conjg(S(6))+conjg(S(7)))*S(1) + conjg(S(1))*(S(4)+S(6)+S(7))), num)
    p(12) = real((i/dsqrt6)*((conjg(S(4))+conjg(S(6))+conjg(S(7)))*S(1) - conjg(S(1))*(S(4)+S(6)+S(7))), num)
    p(13) = real((1./dsqrt2)*(S(1)*conjg(S(8)) + S(8)*conjg(S(1))), num)
    p(14) = real((i/dsqrt2)*(S(1)*conjg(S(8)) - S(8)*conjg(S(1))), num)
    p(15) = real((1./2./dsqrt3)*(conjg(S(4))*(S(2)+S(3)) + conjg(S(6))*(S(2)+S(5)) + conjg(S(7))*(S(3)+S(5)) &
        + conjg(S(2))*(S(4)+S(6)) + conjg(S(3))*(S(4)+S(7)) + conjg(S(5))*(S(6)+S(7))), num)
    p(16) = real((i/2./dsqrt3)*(conjg(S(4))*(S(2)+S(3)) + conjg(S(6))*(S(2)+S(5)) + conjg(S(7))*(S(3)+S(5)) &
        - conjg(S(2))*(S(4)+S(6)) - conjg(S(3))*(S(4)+S(7)) - conjg(S(5))*(S(6)+S(7))), num)
    p(17) = real((1./dsqrt6)*(S(2)*conjg(S(7)) + S(7)*conjg(S(2)) &
        + S(3)*conjg(S(6)) + S(6)*conjg(S(3)) &
        + S(4)*conjg(S(5)) + S(5)*conjg(S(4))), num)
    p(18) = real((i/dsqrt6)*(S(2)*conjg(S(7)) - S(7)*conjg(S(2)) &
        + S(3)*conjg(S(6)) - S(6)*conjg(S(3)) &
        - S(4)*conjg(S(5)) + S(5)*conjg(S(4))), num)
    p(19) = real((1./dsqrt6)*(conjg(S(8))*(S(2)+S(3)+S(5)) + S(8)*(conjg(S(2))+conjg(S(3))+conjg(S(5)))), num)
    p(20) = real((i/dsqrt6)*(conjg(S(8))*(S(2)+S(3)+S(5)) - S(8)*(conjg(S(2))+conjg(S(3))+conjg(S(5)))), num)
    p(21) = real((1./dsqrt6)*(conjg(S(8))*(S(4)+S(6)+S(7)) + S(8)*(conjg(S(4))+conjg(S(6))+conjg(S(7)))), num)
    p(22) = real((i/dsqrt6)*(conjg(S(8))*(S(4)+S(6)+S(7)) - S(8)*(conjg(S(4))+conjg(S(6))+conjg(S(7)))), num)
...
subroutine read_deck
    ...
    integer :: iu
    open(newunit=iu, file='input-T7.deck', status='old', action='read')
