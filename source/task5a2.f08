program task5a2
    ...
    integer, parameter :: r = 5 ! rank / no. chi angles
    real(num), parameter :: dsqrt2 = sqrt(2.0_num)
    ! user-set parameters
    integer :: k ! order+rank (min: 8)
    integer :: na ! no. theta & phi angles (=r*(2*k-r), min: 55)
    ! angle bounds
    ...
    if (k<8) then
        print '(A)', 'Recommended minimum k value (order+rank) is 8.'
...
! $F(\rho)=\sum_i p_i F(\psi_i)$, input angles $(\vec{\theta}_{\rm[na]},\,\vec{\phi}_{\rm[na]},\,\vec{\chi}_{[r=5]})$
...
! initialize the $(\vec{\theta}_{\rm[na]},\,\vec{\phi}_{\rm[na]},\,\vec{\chi}_{[r=5]})$ angles randomly
...
function lambda()
    ...
    lambda(1) = p**4
    lambda(2) = q**4
    lambda(3) = (p**2)*(q**2)*(cos(Theta)**2)
    lambda(4) = 0.25*(p**2)*(q**2)*(3.-cos(2.*Theta)) &
        - 0.25*dsqrt2*sqrt((p**4)*(q**4)*(cos(Theta)**2)*(5.-3.*cos(2.*Theta)))
    lambda(5) = 0.25*(p**2)*(q**2)*(3.-cos(2.*Theta)) &
        + 0.25*dsqrt2*sqrt((p**4)*(q**4)*(cos(Theta)**2)*(5.-3.*cos(2.*Theta)))
...
function vex()
    ...
    vex(1,4) = dsqrt2*(1.0_num/cos(Theta)/sin(Theta) - tan(Theta))
    vex(1,6) = 1.0_num
    vex(1,7) = 1.0_num
    ...
    vex(4,2) = (-6.*(p**2)*(q**2)*(cos(Theta)**2) + dsqrt2*sqrt((p**4)*(q**4)*(cos(Theta)**2)*(5.-3.*cos(2.*Theta))))*tan(Theta)
    vex(4,2) = vex(4,2)/(2.*sqrt((p**4)*(q**4)*(cos(Theta)**2)*(5.-3.*cos(2.*Theta))) &
        + dsqrt2*(p**2)*(q**2)*(-1.+3.*cos(2.*Theta)) + epsilon(0.0_num))
    vex(4,3) = vex(4,2)
    vex(4,5) = 1.0_num

    vex(5,2) = (-6.*(p**2)*(q**2)*(cos(Theta)**2) - dsqrt2*sqrt((p**4)*(q**4)*(cos(Theta)**2)*(5.-3.*cos(2.*Theta))))*tan(Theta)
    vex(5,2) = vex(5,2)/(-2.*sqrt((p**4)*(q**4)*(cos(Theta)**2)*(5.-3.*cos(2.*Theta))) &
        + dsqrt2*(p**2)*(q**2)*(-1.+3.*cos(2.*Theta)) + epsilon(0.0_num))
    vex(5,3) = vex(5,2)
    vex(5,5) = 1.0_num
