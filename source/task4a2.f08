program task4a2
    ...
    integer, parameter :: r = 2 ! rank / no. chi angles
    real(num), parameter :: dsqrt2 = sqrt(2.0_num)
    ! user-set parameters
    integer :: k ! order+rank (min: 5)
    integer :: na ! no. theta & phi angles (=r*(2*k-r), min: 16)
    ! angle bounds
    ...
    if (k<5) then
        print '(A)', 'Recommended minimum k value (order+rank) is 5.'
...
! $F(\rho)=\sum_i p_i F(\psi_i)$, input angles $(\vec{\theta}_{\rm[na]},\,\vec{\phi}_{\rm[na]},\,\vec{\chi}_{[r=2]})$
...
! initialize the $(\vec{\theta}_{\rm[na]},\,\vec{\phi}_{\rm[na]},\,\vec{\chi}_{[r=2]})$ angles randomly
...
function lambda()
    ...
    lambda(1) = exp(-time) ! p^2
    lambda(2) = 1.-exp(-time) ! q^2
...
function vex()
    ...
    vex(1,2) = tan(Theta)/dsqrt2
    vex(1,3) = tan(Theta)/dsqrt2
    vex(1,5) = 1.0_num
