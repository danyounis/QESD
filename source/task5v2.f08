program task5v2
    ...
    integer, parameter :: vdim = 12 ! witness operator dimensionality
    ...
    real(num) :: vrange(2), vstep, wigperc, alpha
    ...
    integer :: l, lm=1
    ! r: rho 12-tuple, v: witness operator 12-tuple
...
subroutine init_exterior(this)
    ...
    if (srestart) then
        ...
        this%p(1,:) = oldp + wigperc*(2.*this%p(1,:)-1.)/100. ! $\vec{v}_0$
        this%y(1) = this%func(this%p(1,:)) ! function value

        ! other vertices obtained by taking unit steps in each direction
        do j=2,vdim+1
            ei = 0.
            ei(j-1) = 1.
            this%p(j,:) = this%p(1,:) + vstep*ei
            this%y(j) = this%func(this%p(j,:))
        end do

    else

        call random_number(this%p(1,:))
        this%p(1,:) = (vrange(2)-vrange(1))*this%p(1,:) + vrange(1) ! $\vec{v}_0$
        this%y(1) = this%func(this%p(1,:)) ! function value
...
function RHO(t)
    ...
    RHO(1) = q**4
    RHO(2) = (p*q*sin(Theta))**2/dsqrt2 + dsqrt2*(p*q*cos(Theta))**2
    RHO(3) = (p**4)*(cos(Theta)**2)
    RHO(4) = (p*q*sin(Theta))**2
    RHO(5) = (p**4)*(sin(Theta)**2)/dsqrt2
    RHO(6) = 0.
    RHO(7) = (p*q*sin(Theta))**2/dsqrt2
    RHO(8) = dsqrt2*sin(Theta)*cos(Theta)*(p**2)*(q**2)
    RHO(9) = (p**4)*(sin(Theta)**2)/dsqrt2
    RHO(10) = dsqrt2*(p**4)*sin(Theta)*cos(Theta)
    RHO(11) = 0.
    RHO(12) = 0.
...
function TrPmatPSI(S) result(p)
    ...
    p(2) = real((abs(S(2))**2 + abs(S(3))**2)/dsqrt2, num)
    p(3) = real(abs(S(4))**2, num)
    p(4) = real(abs(S(5))**2, num)
    p(5) = real((abs(S(6))**2 + abs(S(7))**2)/dsqrt2, num)
    p(6) = real(abs(S(8))**2, num)
    p(7) = real((conjg(S(2))*S(3) + S(2)*conjg(S(3)))/dsqrt2, num)
    p(8) = real((conjg(S(5))*(S(2)+S(3)) + S(5)*(conjg(S(2))+conjg(S(3))))/2., num)
    p(9) = real((S(6)*conjg(S(7)) + conjg(S(6))*S(7))/dsqrt2, num)
    p(10) = real((conjg(S(4))*(S(6)+S(7)) + S(4)*(conjg(S(6))+conjg(S(7))))/2., num)
    p(11) = real((i/2.)*(conjg(S(5))*(S(2)+S(3)) - S(5)*(conjg(S(2))+conjg(S(3)))), num)
    p(12) = real((i/2.)*(S(4)*(conjg(S(6))+conjg(S(7))) - conjg(S(4))*(S(6)+S(7))), num)
...
subroutine read_deck
    ...
    integer :: iu
    open(newunit=iu, file='input-T5.deck', status='old', action='read')
    ...
    read(iu,*) label, vrange(1), vrange(2) ! range to initialize $\vec{v}_0$
    read(iu,*) label, vstep ! direction step-size $\vec{v}_i = \vec{v}_0 + {\rm vstep}\times\vec{e}_i$
    read(iu,*)
