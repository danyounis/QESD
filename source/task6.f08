program task6
    ...
    integer, parameter :: vdim = 10 ! witness operator dimensionality
    ...
    integer :: l, lm=1
    ! r: rho 10-tuple, v: witness operator 10-tuple
...
function RHO(t)
    ...
    RHO(1) = (q**2)*(sin(Theta)**2) + (q**6)*(cos(Theta)**2)
    RHO(2) = (p**2)*(sin(Theta)**2)/dsqrt3 + dsqrt3*(p**2)*(q**4)*(cos(Theta)**2)
    RHO(3) = dsqrt3*(p**4)*(q**2)*(cos(Theta)**2)
    RHO(4) = (p**6)*(cos(Theta)**2)
    RHO(5) = dsqrt6*(p**2)*(sin(Theta)**2)/3.
    RHO(6) = 0.
    RHO(7) = dsqrt2*(p**2)*(q**2)*sin(Theta)*cos(Theta)
    RHO(8) = 0.
    RHO(9) = dsqrt2*(p**4)*sin(Theta)*cos(Theta)
    RHO(10) = 0.
...
function TrPmatPSI(S) result(p)
    ...
    p(7) = real((S(1)*(conjg(S(4))+conjg(S(6))+conjg(S(7))) + conjg(S(1))*(S(4)+S(6)+S(7)))/dsqrt6, num)
    p(8) = real((i/dsqrt6)*(S(1)*(conjg(S(4))+conjg(S(6))+conjg(S(7))) - conjg(S(1))*(S(4)+S(6)+S(7))), num)
    p(9) = real((conjg(S(8))*(S(2)+S(3)+S(5)) + S(8)*(conjg(S(2))+conjg(S(3))+conjg(S(5))))/dsqrt6, num)
    p(10) = real((i/dsqrt6)*(conjg(S(8))*(S(2)+S(3)+S(5)) - S(8)*(conjg(S(2))+conjg(S(3))+conjg(S(5)))), num)
